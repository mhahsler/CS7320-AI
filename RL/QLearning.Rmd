...
QL_Agent$methods(
  ...
  # a more general function would use a function that is increasing in Q 
  # and decreasing in N.
  ...
  run = function(s_prime, r) {
    "Agent function. The pecepts are the current state and the reward."
...
simulate_environment <-
  function(n = 1000, agent,
    ...
    while (TRUE) {
      ...
      action <- agent$run(state, R(state))
      if (verbose)
        cat(action, "\n")
      ...
      if (is_terminal(state)) {
        if (verbose) {
          ...
          cat("U:\n")
          print(show_layout(apply(agent$Q, MARGIN = 1, max)))
        }
...
```{r}
simulate_environment(n = 1, agent, verbose = TRUE)
```

Run a simulation.

```{r}
simulate_environment(n = 10, agent, verbose = FALSE)
```

Calculate the value function $U$ from the learned Q-function as the largest 
Q value of any action in a state.
```{r}
U <- apply(agent$Q, MARGIN = 1, max)
show_layout(U)
```
...
```{r}
Q2 <- agent$Q
Q2[Q2[, 'None'] == 0, 'None'] <- 1e-9
pi <- A[apply(Q2, MARGIN = 1, which.max)]
show_layout(pi)
```

Even after simulating just 10 episodes, we get a reasonable policy that will lead us to the 
goal state. Simulating more episodes results in better estimates (also for the rarely 
visited states).

```{r}
simulate_environment(n = 10000, agent, verbose = FALSE, reset_agent = FALSE)
```
